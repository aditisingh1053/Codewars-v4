...
def ActPirate(pirate):
    ...
    if time < 30 and time>20:
        if int(pirate.getID()) % 7 == 0:
            return 1
        if int(pirate.getID()) % 7 == 1:
            return 2
        if int(pirate.getID()) % 7 == 2:
            return 3
        if int(pirate.getID()) % 7 == 3:
            return 4


    if time >30 and time < 800:
        if int(pirate.getID()) % 7 == (time//timethresh)%7 + 0:
            if time%randthresh == 0:
                return 1
        if int(pirate.getID()) % 7==(time//timethresh)%7 +1:
            if time%randthresh==0:
                return 2
        if int(pirate.getID()) %7 ==(time//timethresh)%7 +2:
            if time%randthresh == 0:
                return 3
        if int(pirate.getID()) %7==(time//timethresh)%7 +3:
            if time%randthresh ==0:
                return 4
...
def ActTeam(team):
    ...
    if team.getCurrentFrame() < 1600:
        team.setTeamSignal("42069")
        #print(team.getCurrentFrame())
    elif team.getCurrentFrame() == 1600:
        team.setTeamSignal("")
    else:
        if s:
            island_no = int(s[0])
